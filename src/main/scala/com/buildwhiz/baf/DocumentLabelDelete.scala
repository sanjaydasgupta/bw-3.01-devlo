  ...
  override def doPost(request: HttpServletRequest, response: HttpServletResponse): Unit = {
    ...
    try {
      ...
      if (!labels.exists(_.name[String] == newLabelName))
        throw new IllegalArgumentException(s"label '$newLabelName' not found")
      val updateResult = BWMongoDB3.persons.updateOne(Map("_id" -> user._id[ObjectId]),
        Map("$pull" -> Map("labels" -> Map("name" -> newLabelName))))
      if (updateResult.getModifiedCount == 0)
        throw new IllegalArgumentException(s"MongoDB update failed: $updateResult")
      response.setStatus(HttpServletResponse.SC_OK)
      BWLogger.audit(getClass.getName, "doPost", s"Deleted document-label $newLabelName", request)
    } catch {
      case t: Throwable =>
