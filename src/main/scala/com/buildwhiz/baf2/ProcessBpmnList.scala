...
class ProcessBpmnList extends HttpServlet with HttpUtils with DateTimeUtils {
  ...
  private def createProcesses(request: HttpServletRequest, activities: Seq[DynDoc], variables: Seq[DynDoc],
        timers: Seq[DynDoc], personOid: ObjectId, process: DynDoc): Seq[DynDoc] = {
  ...
  override def doGet(request: HttpServletRequest, response: HttpServletResponse): Unit = {
    ...
    try {
      ...
      val processes = createProcesses(request, activities, process.variables[Many[Document]], process.timers[Many[Document]],
          personOid, process)
