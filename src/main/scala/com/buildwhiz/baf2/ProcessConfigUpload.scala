...
class ProcessConfigUpload extends HttpServlet with HttpUtils with MailUtils {
  ...
  private def processDocuments(request: HttpServletRequest, response: HttpServletResponse, documentSheet: Sheet,
      processOid: ObjectId, bpmnName: String): String = {
    ...
    def setDocument(docName: String, docDescription: String, labels: String, mandatory: Boolean,
      ...
      if (docRecordExists) {
        ...
        val project: DynDoc = PhaseApi.parentProject(ProcessApi.parentPhase(processOid)._id[ObjectId])
  ...
  private def processVariables(request: HttpServletRequest, response: HttpServletResponse, variableSheet: Sheet,
    ...
    def setVariable(name: String, typ: String, value: String): Unit = {
      ...
      val variables: Option[DynDoc] = process.variables[Many[Document]].
  ...
  private def processTimers(request: HttpServletRequest, response: HttpServletResponse, timersSheet: Sheet,
    ...
    def setTimer(name: String, typ: String, duration: String): Unit = {
      val process: DynDoc = ProcessApi.processById(processOid)
      val timers: Option[DynDoc] = process.timers[Many[Document]].
  ...
  private def processObserverRoles(request: HttpServletRequest, response: HttpServletResponse, timersSheet: Sheet,
    ...
    def setRole(roleName: String, personName: String, personId: String): Unit = {
      ...
      val process: DynDoc = ProcessApi.processById(processOid)
      val assignedRoles: Seq[DynDoc] = if (process.has("assigned_roles"))
        process.assigned_roles[Many[Document]] else Seq.empty[DynDoc]
      val updateResult = assignedRoles.indexWhere(_.role_name[String] == roleName) match {
        case -1 =>
          ...
          BWMongoDB3.processes.updateOne(Map("_id" -> processOid),
  ...
  private def doPostTransaction(request: HttpServletRequest, response: HttpServletResponse): Unit = {
    ...
    try {
      ...
      val itemsUploaded: String = sheets.map(s => (s.getSheetName, s)).map({
        ...
        case ("Variables", sheet) => processVariables(request, response, sheet, processOid, bpmnName)
        case ("Timers", sheet) => processTimers(request, response, sheet, processOid, bpmnName)
        case ("Documents", sheet) => processDocuments(request, response, sheet, processOid, bpmnName)
