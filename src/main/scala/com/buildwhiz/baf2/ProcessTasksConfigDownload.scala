...
class ProcessTasksConfigDownload extends HttpServlet with HttpUtils {
  ...
  private def addDocumentsSheet(workbook: XSSFWorkbook, process: DynDoc, bpmnName: String): Int = {
  ...
  private def addObserverRolesSheet(workbook: XSSFWorkbook, process: DynDoc, bpmnName: String): Int = {
  ...
  override def doGet(request: HttpServletRequest, response: HttpServletResponse): Unit = {
    ...
    try {
      ...
      //val docNbr = addDocumentsSheet(workbook, process, bpmnName)
      //val roleNbr = addObserverRolesSheet(workbook, process, bpmnName)
      workbook.write(response.getOutputStream)
      response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
      response.setStatus(HttpServletResponse.SC_OK)
      BWLogger.log(getClass.getName, request.getMethod,
        s"EXIT-OK ($taskNbr tasks, $varNbr variables, $timerNbr timers)", request)
    } catch {
      case t: Throwable =>
