...
object ActivityInfo extends DateTimeUtils {
  ...
  private def activity2json(activity: DynDoc, user: DynDoc): String = {
    ...
    val latestStart = new Document("editable", editable).append("value", activity.latest_start[String])
    val dateStart = new Document("editable", editable).append("value", activity.date_start[String])
    val dateEnd = new Document("editable", editable).append("value", activity.date_end[String])
