...
class MSLogin extends HttpServlet with HttpUtils with DateTimeUtils {
        ...
        if (idTokenOk) {
          ...
          val result = person match {
            ...
            case Some(personRecord) =>
              ...
              BWLogger.audit(getClass.getName, request.getMethod, "Login MicrosoftIdTokenVerifier OK", request)
              bson2json(resultPerson)
          }
          response.getWriter.print(result)
        } else {
          BWLogger.log(getClass.getName, request.getMethod, "EXIT-ERROR MicrosoftIdTokenVerifier failure", request)
