...
object BWLogger {
  ...
  private def log(className: String, methodName: String, eventName: String, variables: JMap[String, AnyRef]): Unit =
    log(className, methodName, eventName, (if (variables == null) Nil else
      variables.toSeq.map(p => (p._1, if (p._2 == null) "" else p._2.toString))): _*)
  ...
  def log(className: String, methodName: String, eventName: String, request: HttpServletRequest): Unit = {
    ...
    if (eventName.toLowerCase.contains("entry")) {
      val referrer = if (request.getHeaderNames.contains("referer")) request.getHeader("referer") else "(none)"
      ...
      log(className, methodName,
        s"""$eventName ($path${if (query == null) "" else "?" + query}) client=$clientIp referrer=$refPage""", parameters)
    } else {
      log(className, methodName, s"""$eventName client=$clientIp""", parameters)
    }
  }
