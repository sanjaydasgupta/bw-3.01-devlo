...
object DocumentCategoryRecordsRedo extends App {
  ...
  private def store(categories: Seq[(String, Seq[(String, String)])]): Unit = {
    val withIds: Seq[(String, Seq[ObjectId])] = categories.map(c => {
      (c._1, c._2.map(c2 => {
        BWMongoDB3.roles_master.find(Map("category" -> c2._1, "name" -> c2._2)).asScala.head.y._id[ObjectId]}))
    })
    val docsWithIds = withIds.map(d => {
      val doc = new Document("category", d._1)
      doc.put("roleIds", d._2.asJava)
      doc
    })

    println(s"Original document_categories count: ${BWMongoDB3.roles_master.count()}")
    BWMongoDB3.document_category_master.insertMany(docsWithIds.asJava)
    println(s"Final document_categories count: ${BWMongoDB3.roles_master.count()}")
  }
