...
object Project430RecordsRedo extends App {
  ...
  private def createActivity(): ObjectId = {
    ...
    BWMongoDB3.processes.insertOne(defaultPhase)
    val thePhase: DynDoc = BWMongoDB3.processes.find(Map("name" -> defaultName)).head
  ...
  private def createProject(): Unit = {
    ...
    val forest430: Document = Map("_id" -> project430ForestOid, "name" -> defaultName,
      "timestamps" -> Map("created" -> now, "start" -> now), "process_ids" -> Seq(phaseOid).asJava,
