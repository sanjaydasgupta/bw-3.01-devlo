...
class PrerequisiteRetest extends JavaDelegate {
  ...
  def execute(de: DelegateExecution): Unit = {
    ...
    try {
      ...
      val activity: Document = BWMongoDB3.activities.find(query).head
      val actions: Seq[Document] = activity("actions").asInstanceOf[DocumentList]
      val actionsWithIndex = actions.zipWithIndex
      val prerequisiteName = de.getVariable("action_name")
      actionsWithIndex.find(a => a._1("name") == prerequisiteName && a._1("type") == "prerequisite") match {
        case Some(awi) if awi._1("status") == "ready" =>
          val id = awi._1("id").asInstanceOf[String]
        ...
        case Some(awi) =>
          BWLogger.log(getClass.getName, "execute()",
            s"Prerequisite '$prerequisiteName' status: ${awi._1("status")}, taking no action", de)
      }
