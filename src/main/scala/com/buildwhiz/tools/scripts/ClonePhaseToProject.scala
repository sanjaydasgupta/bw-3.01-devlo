...
@unused
object ClonePhaseToProject extends HttpUtils {
      ...
      if (args.length >= 2) {
        ...
        BWMongoDB3.phases.find(Map("_id" -> phaseSourceOid)).headOption match {
          ...
          case Some(phaseSource) =>
            ...
            val projectDestOid = new ObjectId(args(1))
            BWMongoDB3.projects.find(Map("_id" -> projectDestOid)).headOption match {
              case None =>
                output(s"""<font color="red">No such project ID: '$projectDestOid'</font><br/>""")
              case Some(projectDest) =>
                output(s"Found dest project: '${projectDest.name[String]}'<br/>")
