...
.controller("ProjectsCtrl", ['$log', '$http', 'AuthenticationService', '$routeParams', '$window',
  ...
  self.createNewProject = function() {
    ...
    var q = '/bw-3.01/baf2/ProjectCreate?name=' + self.newProjectName;
    self.busy = true;
    $http.post(q).then(
      function(resp) {
        self.busy = false;
  ...
  self.projectPublicChanged = function() {
    ...
    var query = '/bw-3.01/baf/ProjectSetPublic?project_id=' + self.selectedProject._id + '&public=' + self.selectedProject.public;
    self.busy = true;
  ...
  self.launchSelectedProject = function() {
    ...
    var query = '/bw-3.01/baf/ProjectLaunch?project_id=' + project._id;
  ...
  self.endSelectedProject = function() {
    var project = self.selectedProject;
    var query = '/bw-3.01/baf2/ProjectEnd?project_id=' + project._id;
    $log.log('calling POST ' + query)
    self.busy = true;
    $http.post(query).then(
      function(resp) {
  ...
  self.deleteSelectedProject = function() {
    var project = self.selectedProject;
    var query = '/bw-3.01/baf2/ProjectDelete?project_id=' + project._id;
    $log.log('calling POST ' + query)
    self.busy = true;
    $http.post(query).then(
      function(resp) {
...
//        '&project_id=' + self.selectedProject._id;
    var query = '/bw-3.01/baf2/ProjectAdminSet?person_id=' + self.selectedProjectManager._id +
        '&project_id=' + self.selectedProject._id;
    self.busy = true;
    $http.post(query).then(
      function(resp) {
  ...
  self.addNewPhase = function() {
    ...
    var query = '/bw-3.01/baf2/PhaseAdd?project_id=' + self.selectedProject._id + '&phase_name=' + self.newPhaseName;
    self.busy = true;
    $http.post(query).then(
      function(resp) {
        self.busy = false;
  ...
  self.deleteSelectedPhase = function() {
    ...
    var query = '/bw-3.01/baf2/PhaseDelete?phase_id=' + phase._id;
    $log.log('calling POST ' + query)
    self.busy = true;
    $http.post(query).then(
      function(resp) {
  ...
  self.addProcess = function(name) {
    ...
    var query = '/bw-3.01/baf2/ProcessAdd?bpmn_name=' + self.selectedBpmnName + '&phase_id=' + self.selectedPhase._id +
  ...
  self.selectedProcessEnd = function(name) {
    var query = '/bw-3.01/baf/PhaseEnd?phase_id=' + self.selectedPhase._id;
    self.busy = true;
    $http.post(query).then(
      function(resp) {
        self.busy = false;
...
//        '&project_id=' + self.selectedProject._id + '&phase_id=' + self.selectedPhase._id;
    var query = '/bw-3.01/baf2/PhaseAdminSet?person_id=' + self.selectedPhaseManager._id +
        '&phase_id=' + self.selectedPhase._id;
    self.busy = true;
    $http.post(query).then(
      function(resp) {
  ...
  self.selectedProcessLaunch = function() {
    ...
    var query = '/bw-3.01/baf2/ProcessLaunch?process_id=' + self.selectedProcess._id;
    $log.log('calling POST ' + query)
    self.busy = true;
    $http.post(query).then(
      function(resp) {
  ...
  self.selectedProcessDelete = function() {
    ...
    var query = '/bw-3.01/baf2/ProcessDelete?process_id=' + self.selectedProcess._id;
    $log.log('calling POST ' + query)
    self.busy = true;
    $http.post(query).then(
      function(resp) {
  ...
  self.uploadBegin = function(evt, bpmn_name) {
    ...
    if (files.length > 0) {
      ...
      var query = '/bw-3.01/baf/PhaseConfigUpload?project_id=' + self.selectedProject._id +
