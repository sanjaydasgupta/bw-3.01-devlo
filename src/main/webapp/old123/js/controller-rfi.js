...
//origin0=documents-view2&origin1=Architecture&origin2=Sections&origin3=Sheet-4F
  ...
  self.submitRFI = function() {
    var query = '/bw-3.01/baf/RFIMessageSubmit';
  ...
  self.attachmentUploadBegin = function(evt) {
    ...
    if (files.length > 0) {
      ...
      var query = '/bw-3.01/baf/DocumentPreload?person_id=' + AuthService.data._id + '&timestamp=' + timestamp +
  ...
  self.closeMessage = function() {
    ...
    var q = '/bw-3.01/baf/RFIClose?rfi_id=' + self.selectedMessage._id;
     self.busy = true;
    $http.post(q).then(
      function(resp) {
        self.busy = false;
