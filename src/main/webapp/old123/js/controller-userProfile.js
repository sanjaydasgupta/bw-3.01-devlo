...
.controller("UserProfileCtrl", ['$log', '$http', 'AuthenticationService', function ($log, $http, AuthService) {
  ...
  self.addNewLabelName = function() {
    var query = '/bw-3.01/baf/DocumentLabelAdd?label_name=' + self.newLabelName;
  ...
  self.deleteLabelName = function() {
    var query = '/bw-3.01/baf/DocumentLabelDelete?label_name=' + self.selectedLabel.name;
  ...
  self.userPasswordSet = function() {
    ...
    $http.post('/bw-3.01/baf/UserPasswordSet', postData).then(
      function() {
        self.oldPassword = '';
          self.newPassword = '';
          self.cnfPassword = '';
  ...
  self.toggleEmail = function() {
    var query = '/bw-3.01/baf/UserPropertySet?person_id=' + AuthService.data._id + '&property=email_enabled&value=' +
        self.emailEnabled;
    $log.log('POST ' + query);
    $http.post(query).then(
      function(res) {
